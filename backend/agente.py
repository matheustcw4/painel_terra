...
async def perguntar_streaming(mensagem: str, historico: list):
    ...
    if todas_tools is None:
        _log("chat: nenhum MCP configurado")
        yield "Nenhum MCP server configurado ainda — confere o .env do backend."
        return

    base_escolhida = await _escolher_base(mensagem, nomes, historico)
    if base_escolhida == "fora de contexto":
        ferramentas = []
        _log("resposta: fora de contexto — sem ferramentas")
    else:
        ferramentas = _ferramentas_da_base(todas_tools, base_escolhida)
        _log(f"resposta: iniciando com {len(ferramentas)} ferramenta(s) — {[t.name for t in ferramentas]}")

    modelo = _modelo_deepseek(MODELO)  # <- MODELO aqui, não MODELO_ROTEADOR
    agente = create_react_agent(modelo, ferramentas, prompt=SYSTEM_PROMPT)
